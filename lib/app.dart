      ...
      child: CustodeMontato(
        ...
        child: CustodeDellaMusica(
          ...
          child: PonteDellaCarta(
            // **LA LINGUA GOVERNA L'APP INTERA, ordine DM voce 02.**
            //
            // **Il difetto che questo cura, e c'era in italiano**: senza i
            // delegati i widget che Flutter disegna da se' parlano inglese.
            // Si vede in tre punti, misurati: i due selettori di data del
            // profilo e del Sigillo, e quello dell'ora delle notifiche. Chi
            // sceglieva la propria data di nascita leggeva CANCEL e OK.
            //
            // **Si aggancia al `ValueListenable` della porta della lingua**,
            // non a un provider: cosi' un cambio di lingua ridisegna tutto
            // senza che nessuna schermata debba ricordarsene, e i corpora,
            // che un `BuildContext` non ce l'hanno, leggono la stessa porta.
            child: ValueListenableBuilder<LinguaDelCerchio>(
              valueListenable: LaLinguaDelCerchio.corrente,
              builder: (context, lingua, _) => MaterialApp(
                navigatorKey: _navigatore,
                title: 'Esoteric Circle',
                debugShowCheckedModeBanner: false,
                theme: AppTheme.dark(),
                // **LA LINGUA E' DICHIARATA, NON DEDOTTA DAL TELEFONO.** Con
                // `locale` nullo Flutter sceglierebbe la lingua di sistema, e
                // un telefono in inglese metterebbe in inglese i selettori di
                // chi ha sempre letto in italiano: sarebbe un comportamento
                // visibile cambiato, che quest'ordine vieta.
                locale: Locale(lingua.codice),
                supportedLocales: [
