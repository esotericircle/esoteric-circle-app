...
/// LA GUARDIA DELL'ORDINE BJ.
///
/// **Non e' una promessa, e' un test che non passa.** Legge il manifesto e
/// resta rossa finche' le tre voci non hanno uno stato terminale. Nasce dal
/// collaudo della 2204: i fatti misurati stanno nel manifesto.
void main() {
  final manifesto = File('docs/ordini/ORDINE_BJ_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*BJ\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e tre le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_BJ_MANIFESTO.md non esiste');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 0; i < quante; i++) {
      final voce = 'BJ.${i.toString().padLeft(2, '0')}';
  ...
  test('l\'ordine BJ non e\' finito finche\' una voce resta aperta', () {
    final aperte = marcatore(manifesto.readAsStringSync(), 'VOCI_APERTE');
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine BJ. Questa riga '
