...
void main() {
  final manifesto = File('docs/ordini/ORDINE_AM_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*AM\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e nove le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_AM_MANIFESTO.md non esiste, e la legge di '
            'consegna dice che nasce prima del codice');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 1; i <= quante; i++) {
      final voce = 'AM.${i.toString().padLeft(2, '0')}';
  ...
  test('ogni voce ha uno stato, e almeno uno dei quattro ammessi', () {
    ...
    // ignore: avoid_print
    print('ORDINE AM: righe di voce osservate $osservate');
  ...
  test('l\'ordine AM non e\' finito finche\' una voce resta aperta', () {
    final testo = manifesto.readAsStringSync();
    final aperte = marcatore(testo, 'VOCI_APERTE');
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine AM. Questa riga e\' '
