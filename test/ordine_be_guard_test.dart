...
/// LA GUARDIA DELL'ORDINE BE.
///
/// **Non e' una promessa, e' un test che non passa.** Legge il manifesto e
/// resta rossa finche' le dieci voci non hanno uno stato terminale.
///
/// **Nessun censimento da sorvegliare qui dentro.** Le nove voci di
/// quest'ordine hanno ciascuna la propria prova: la card vecchia demolita,
/// la bolla dei sentieri, la rotta senza nome dell'onboarding, la
/// fluttuazione dei Maestri, l'Alba leggibile, le nazioni vere, la
/// cancellazione immediata e il censimento anti abuso. Questa guardia
/// sorveglia solo che nessuna resti aperta.
void main() {
  final manifesto = File('docs/ordini/ORDINE_BE_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*BE\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e dieci le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_BE_MANIFESTO.md non esiste, e la legge di '
            'consegna dice che nasce prima del codice');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 0; i < quante; i++) {
      final voce = 'BE.${i.toString().padLeft(2, '0')}';
  ...
  test('ogni voce ha uno stato, e almeno uno dei cinque ammessi', () {
    ...
    // ignore: avoid_print
    print('ORDINE BE: righe di voce osservate ${righe.length}');
  });
  ...
  test('l\'ordine BE non e\' finito finche\' una voce resta aperta', () {
    final aperte = marcatore(manifesto.readAsStringSync(), 'VOCI_APERTE');
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine BE. Questa riga e\' '
            'rossa apposta e non si tocca: torna verde quando le dieci voci '
