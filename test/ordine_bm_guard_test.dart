...
/// LA GUARDIA DELL'ORDINE BM.
///
/// **Non e' una promessa, e' un test che non passa.** Legge il manifesto e
/// resta rossa finche' le cinque voci non hanno uno stato terminale. Nasce dal
/// esito dell'ordine BK: i fatti misurati stanno nel manifesto, insieme
/// alle sei premesse verificate vere prima di eseguire.
void main() {
  final manifesto = File('docs/ordini/ORDINE_BM_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*BM\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e cinque le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_BM_MANIFESTO.md non esiste');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 0; i < quante; i++) {
      final voce = 'BM.${i.toString().padLeft(2, '0')}';
  ...
  test('l\'ordine BM non e\' finito finche\' una voce resta aperta', () {
    final aperte = marcatore(manifesto.readAsStringSync(), 'VOCI_APERTE');
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine BM. Questa riga '
