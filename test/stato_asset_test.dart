...
void main() {
  ...
  test('pubspec dichiara esattamente gli asset attesi dal manifest', () {
    ...
    final dichiarati = ((pubspec['flutter'] as YamlMap)['assets'] as YamlList)
        .map((e) => e.toString())
        .toSet();
    final attesi = (bundle['pubspec_asset_attesi'] as List)
        .map((e) => e.toString())
        .toSet();
    expect(dichiarati, attesi,
        reason:
            'pubspec.yaml e docs/stato_asset.json non coincidono. Allineali insieme.');
  });
  ...
  test('i conteggi file in brand_assets coincidono col manifest', () {
    ...
    conteggi.forEach((cartella, atteso) {
      ...
      final reale =
          dir.existsSync() ? dir.listSync().whereType<File>().length : -1;
      expect(reale, atteso as int,
          reason:
              'brand_assets/$cartella ha $reale file, il manifest ne dichiara $atteso. Aggiorna docs/stato_asset.json.');
    });
  });
