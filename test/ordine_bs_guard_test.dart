...
/// LA GUARDIA DELL'ORDINE BS.
///
/// **Non e' una promessa, e' un test che non passa.** Legge il manifesto e resta
/// rossa finche' le quattro voci non hanno uno stato terminale. Lo stato di una voce
...
/// di stato: una voce che manda il suo **CHIUSA** a capo non viene contata e la
/// guardia cade sul marcatore. E' una convenzione stretta apposta: contare su
/// tutto il blocco vorrebbe dire contare anche le parole di stato nominate in
/// mezzo alla prosa.
void main() {
  final manifesto = File('docs/ordini/ORDINE_BS_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*BS\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e quattro le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_BS_MANIFESTO.md non esiste');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 0; i < quante; i++) {
      final voce = 'BS.${i.toString().padLeft(2, '0')}';
  ...
  test('l\'ordine BS non e\' finito finche\' una voce resta aperta', () {
    final aperte = marcatore(manifesto.readAsStringSync(), 'VOCI_APERTE');
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine BS. Questa riga '
