...
/// LA GUARDIA DELL'ORDINE BH.
///
/// **Non e' una promessa, e' un test che non passa.** Legge il manifesto e
/// resta rossa finche' le nove voci non hanno uno stato terminale. L'ordine e'
/// arrivato in due tempi (il blocco esplicito del fondatore a meta'): la
/// cronaca sta nel manifesto.
void main() {
  final manifesto = File('docs/ordini/ORDINE_BH_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*BH\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e nove le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_BH_MANIFESTO.md non esiste');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 0; i < quante; i++) {
      final voce = 'BH.${i.toString().padLeft(2, '0')}';
  ...
  test('l\'ordine BH non e\' finito finche\' una voce resta aperta', () {
    final aperte = marcatore(manifesto.readAsStringSync(), 'VOCI_APERTE');
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine BH. Questa riga '
