...
/// LA GUARDIA DELL'ORDINE ED, il collaudo sui tre Maestri.
///
...
///
/// **Le voci di questo manifesto sono intestazioni**, `## VOCE ED.NN, ...`, e
/// una voce comincia li' e finisce alla prossima intestazione di pari grado.
void main() {
  final manifesto = File('docs/ordini/ORDINE_ED_MANIFESTO.md');
  ...
  test('il manifesto esiste e porta tutte e cinque le voci', () {
    ...
    for (var i = 1; i <= quante; i++) {
      final voce = 'ED.${i.toString().padLeft(2, '0')}';
  ...
  test('il manifesto porta i censimenti che l\'ordine pretende', () {
    ...
    final pretese = <String, String>{
      ...
      'docs/collaudo/ED/': 'le trascrizioni della voce 01 non hanno una casa '
          'dichiarata',
      'lib/services/app_services.dart:115': 'la voce 03 deve dire col file e '
          'la riga dove la rete si costruisce, e dichiararlo senza il numero '
          'di riga vuol dire non averlo verificato',
      'art_catalog.dart': 'gli scarti sulle mosse 5 e 14 poggiano sul '
          'catalogo delle arti: senza il file, sono un\'opinione',
      'feature_catalog.dart': 'lo scarto sulla mossa 14 poggia sullo stato '
          'Coming soon della Chiromanzia, che sta li\'',
    };
    final mancanti = <String>[];
    pretese.forEach((pezzo, perche) {
      if (!testo.contains(pezzo)) mancanti.add(perche);
    });
    // **Le sedici mosse nelle tre dizioni**, una riga di tabella ciascuna:
    // e' il censimento che la voce ED.01 pretende per intero. Senza questo
    ...
    final mosse = RegExp(r'^\| \d+ \| ', multiLine: true)
        ...
        .length;
    print('ORDINE ED: righe numerate nelle tabelle del manifesto $mosse');
    if (mosse < 16) {
      mancanti.add('le tabelle numerate del manifesto portano $mosse righe: '
          'le mosse del catalogo di EB sono sedici, e ognuna deve dire come '
          'si dice per ciascuno dei tre Maestri');
    }
  ...
  test('i marcatori dicono il vero, contati sulle voci', () {
    ...
    for (final r in righe) {
      if (RegExp(r'^## VOCE ED\.\d\d,').hasMatch(r)) {
    ...
    print('ORDINE ED: voci osservate ${voci.length}, chiuse $chiuse, '
  ...
  test('l\'ordine ED non e\' finito finche\' una voce resta aperta', () {
    ...
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE. Questa riga e\' rossa apposta: '
            'torna verde quando le cinque voci hanno uno stato terminale');
  });
}
