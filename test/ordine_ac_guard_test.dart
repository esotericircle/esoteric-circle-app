...
/// LA GUARDIA DELL'ORDINE AC.
///
/// **Non e' una promessa, e' un test che non passa.** Legge il manifesto e resta
/// rossa finche' tutte e dodici le voci non hanno uno stato terminale.
///
/// **Nasce prima del codice, ed e' la legge di consegna di questo ordine.** Un
/// manifesto scritto dopo racconta cio' che si e' fatto; scritto prima, dice cio'
/// che si e' promesso, e la differenza si vede solo quando una voce non si riesce
/// a chiudere.
///
/// Si committa comunque a ogni voce chiusa, con la guardia rossa o verde:
/// committare non e' consegnare.
void main() {
  final manifesto = File('docs/ordini/ORDINE_AC_MANIFESTO.md');

  /// **QUANTE VOCI HA QUESTO ORDINE, ed e' un dato e non una stima.** Le voci
  /// non si rinumerano, non si accorpano e non si dichiarano coperte da
  /// un'altra: se un giorno questo numero cambiasse, cambierebbe l'ordine, non
  /// la guardia.
  const quante = 12;
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*AC\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e dodici le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_AC_MANIFESTO.md non esiste, e la legge di '
            'consegna di questo ordine dice che nasce prima del codice');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 1; i <= quante; i++) {
      final voce = 'AC.${i.toString().padLeft(2, '0')}';
  ...
  test('ogni voce ha uno stato, e almeno uno dei quattro ammessi', () {
    ...
    // ignore: avoid_print
    print('ORDINE AC: righe di voce osservate $osservate');
    expect(osservate, quante,
        reason: 'la prova ha guardato $osservate righe invece di $quante');
  });
  ...
  test('l\'ordine AC non e\' finito finche\' una voce resta aperta', () {
    final testo = manifesto.readAsStringSync();
    final aperte = marcatore(testo, 'VOCI_APERTE');
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine AC. Questa riga e\' '
            'rossa apposta e non si tocca: torna verde quando la somma degli '
            'stati terminali fa $quante, e non prima');
  });
}
