...
/// LA GUARDIA DELL'ORDINE AR.
///
/// **Non e' una promessa, e' un test che non passa.** Legge il manifesto e resta
/// rossa finche' le sei voci non hanno uno stato terminale.
///
...
void main() {
  final manifesto = File('docs/ordini/ORDINE_AR_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*AR\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e undici le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_AR_MANIFESTO.md non esiste, e la legge di '
            'consegna dice che nasce prima del codice');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 1; i <= quante; i++) {
      final voce = 'AR.${i.toString().padLeft(2, '0')}';
  ...
  test('ogni voce ha uno stato, e almeno uno dei quattro ammessi', () {
    ...
    // ignore: avoid_print
    print('ORDINE AR: righe di voce osservate $osservate');
  ...
  test('l\'ordine AR non e\' finito finche\' una voce resta aperta', () {
    final testo = manifesto.readAsStringSync();
    final aperte = marcatore(testo, 'VOCI_APERTE');
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine AR. Questa riga e\' '
            'rossa apposta e non si tocca: torna verde quando le undici voci hanno '
