...
/// LA GUARDIA DELL'ORDINE CE.
///
/// **Non e' una promessa, e' un test che non passa.** Legge il manifesto e resta
/// rossa finche' le diciassette voci non hanno uno stato terminale.
///
...
void main() {
  final manifesto = File('docs/ordini/ORDINE_CE_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*CE\.\d\d\*\*').hasMatch(r))
      .toList();
  ...
  test('il manifesto esiste e nomina tutte e diciassette le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_CE_MANIFESTO.md non esiste');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 0; i < quante; i++) {
      final voce = 'CE.${(i + 1).toString().padLeft(2, '0')}';
  ...
  test('le affermazioni false sono elencate col fatto vero accanto', () {
    ...
    final false_ = RegExp(r'\*\*(VERA A META\x27|SUPERATA|FALSA)')
        ...
        .length;
    // ignore: avoid_print
    print('ORDINE CE: affermazioni dell\'ordine non vere e dichiarate '
        '$false_');
