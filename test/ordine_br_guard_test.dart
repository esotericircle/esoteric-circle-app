...
/// LA GUARDIA DELL'ORDINE BR.
///
/// **Non e' una promessa, e' un test che non passa.** Legge il manifesto e resta
/// rossa finche' le tre voci non hanno uno stato terminale. Lo stato di una voce
...
///
/// **LO STATO STA SULLA RIGA DELLA VOCE, non due righe piu' sotto.** Questa
/// guardia legge la riga che comincia col nome della voce e cerca li' la parola
/// di stato: una voce che manda il suo **CHIUSA** a capo non viene contata, la
/// guardia cade sul marcatore, ed e' successo mentre si scriveva questo
/// manifesto. E' una convenzione stretta apposta: contare su tutto il blocco
/// vorrebbe dire contare anche le parole di stato nominate in mezzo alla prosa.
///
/// **I marcatori e le righe si contano INSIEME.** Un marcatore che dice quattro
/// voci chiuse mentre le righe ne dichiarano tre e' una bugia scritta a mano, e
/// questa guardia cade prima che qualcuno la legga.
void main() {
  final manifesto = File('docs/ordini/ORDINE_BR_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*BR\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e tre le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_BR_MANIFESTO.md non esiste');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 0; i < quante; i++) {
      final voce = 'BR.${i.toString().padLeft(2, '0')}';
  ...
  test('l\'ordine BR non e\' finito finche\' una voce resta aperta', () {
    final aperte = marcatore(manifesto.readAsStringSync(), 'VOCI_APERTE');
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine BR. Questa riga '
