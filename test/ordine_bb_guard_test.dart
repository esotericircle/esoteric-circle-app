...
/// sarebbe rumore.
void main() {
  final manifesto = File('docs/ordini/ORDINE_BB_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*BB\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e quattordici le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_BB_MANIFESTO.md non esiste, e la legge di '
            'consegna dice che nasce prima del codice');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 0; i < quante; i++) {
      final voce = 'BB.${i.toString().padLeft(2, '0')}';
  ...
  test('ogni voce ha uno stato, e almeno uno dei cinque ammessi', () {
    ...
    // ignore: avoid_print
    print('ORDINE BB: righe di voce osservate ${righe.length}');
  });
  ...
  test('l\'ordine BB non e\' finito finche\' una voce resta aperta', () {
    final aperte = marcatore(manifesto.readAsStringSync(), 'VOCI_APERTE');
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine BB. Questa riga e\' '
