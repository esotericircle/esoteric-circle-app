...
/// LA GUARDIA DELL'ORDINE AS.
///
/// **Non e' una promessa, e' un test che non passa.** Legge il manifesto e resta
/// rossa finche' le dodici voci non hanno uno stato terminale.
///
...
void main() {
  final manifesto = File('docs/ordini/ORDINE_AS_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*AS\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e dodici le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_AS_MANIFESTO.md non esiste, e la legge di '
            'consegna dice che nasce prima del codice');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 1; i <= quante; i++) {
      final voce = 'AS.${i.toString().padLeft(2, '0')}';
  ...
  test('l\'ordine AS non e\' finito finche\' una voce resta aperta', () {
    ...
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine AS. Questa riga e\' '
            'rossa apposta e non si tocca: torna verde quando le dodici voci '
