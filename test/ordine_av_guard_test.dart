...
void main() {
  final manifesto = File('docs/ordini/ORDINE_AV_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*AV\.\d\d\*\*').hasMatch(r))
      .toList();

  test('il manifesto esiste e porta tutte e cinque le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_AV_MANIFESTO.md non esiste, e la legge di '
            'consegna dice che nasce prima del codice');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 0; i < quante; i++) {
      final voce = 'AV.${i.toString().padLeft(2, '0')}';
  ...
  test('ogni voce ha uno stato, e almeno uno dei quattro ammessi', () {
    ...
    // ignore: avoid_print
    print('ORDINE AV: righe di voce osservate $osservate');
  ...
  test('l\'ordine AV non e\' finito finche\' una voce resta aperta', () {
    final testo = manifesto.readAsStringSync();
    final aperte = marcatore(testo, 'VOCI_APERTE');
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine AV. Questa riga e\' '
            'rossa apposta e non si tocca: torna verde quando le cinque voci '
