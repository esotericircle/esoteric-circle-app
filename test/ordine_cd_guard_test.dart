...
/// LA GUARDIA DELL'ORDINE CD.
///
/// **Non e' una promessa, e' un test che non passa.** Legge il manifesto e resta
/// rossa finche' le due voci non hanno uno stato terminale.
///
/// **E chiede tre sezioni**, perche' quest'ordine le pretende per nome: le
/// affermazioni trovate false, cosa era GIA' fatto prima di cominciare, e le
/// scelte prese da me con la loro ragione.
///
/// La seconda e' la piu' importante di quest'ordine: le sue due voci ripetono
/// parola per parola richieste che l'ordine CC aveva gia' chiuso, e il
/// fondatore vuole sapere cosa ha gia' e cosa no.
void main() {
  final manifesto = File('docs/ordini/ORDINE_CD_MANIFESTO.md');
  ...
  List<String> righeDiVoce(String testo) => testo
      .split('\n')
      .where((r) => RegExp(r'^- \*\*CD\.\d\d\*\*').hasMatch(r))
      .toList();
  ...
  test('il manifesto esiste e nomina tutte e due le voci', () {
    expect(manifesto.existsSync(), isTrue,
        reason: 'docs/ordini/ORDINE_CD_MANIFESTO.md non esiste');
    final testo = manifesto.readAsStringSync();
    final mancanti = <String>[];
    for (var i = 0; i < quante; i++) {
      final voce = 'CD.${(i + 1).toString().padLeft(2, '0')}';
  ...
  test('le tre sezioni obbligatorie ci sono, e non sono vuote', () {
    ...
    const sezioni = <String, String>{
      ...
      'COSA ERA GIA\' FATTO':
          'serve al fondatore per sapere cosa ha gia\' e cosa no, '
  ...
  test('le affermazioni false sono elencate col fatto vero accanto', () {
    ...
    final false_ = RegExp(r'\*\*(VERA A META\x27|SUPERATA|FALSA)')
        ...
        .length;
    // ignore: avoid_print
    print('ORDINE CD: affermazioni dell\'ordine non vere e dichiarate '
        '$false_');
    expect(false_, greaterThanOrEqualTo(1),
  ...
  test('l\'ordine CC non e\' finito finche\' una voce resta aperta', () {
    ...
    expect(aperte, 0,
        reason: 'restano $aperte voci APERTE nell\'ordine CD. Questa riga '
