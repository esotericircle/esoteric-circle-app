...
void main() {
  ...
  group('Strato dinamico: cambiando l\'input, l\'output cambia', () {
    ...
    test('Cielo: due luoghi diversi danno due cieli diversi', () {
      ...
      expect(
          (stellaDaMilano.altDeg - stellaDaSydney.altDeg).abs(), greaterThan(5),
    ...
    test('Cielo: due istanti diversi danno due cieli diversi', () {
      ...
      expect(sera.centerAzDeg, isNot(alba.centerAzDeg),
          reason:
              'a nove ore di distanza il cielo risulta identico: l\'ora non '
              'entra nel calcolo');
    });
    ...
    test('Illuminazione: l\'elongazione cambia col tempo', () {
      final a = Celestial.moonIllumination(
          Celestial.julianDay(DateTime.utc(2026, 3, 1)));
      final b = Celestial.moonIllumination(
          Celestial.julianDay(DateTime.utc(2026, 3, 8)));
    ...
    test('Angeli: due nascite diverse danno triadi diverse', () {
      final a =
          GuardianAngels.forBirth(BirthDetails(date: DateTime(1990, 5, 12)));
      final b =
          GuardianAngels.forBirth(BirthDetails(date: DateTime(1990, 11, 12)));
    ...
    test('Angeli: l\'ora di nascita conta dove deve contare', () {
      ...
      expect(senza.known.length, isNot(con.known.length),
          reason:
              'con e senza ora di nascita la triade e\' identica, mentre il '
...
const List<Motore> censimentoMotori = [
  Motore(
      'Carta natale, client', 'FreeAstroClient', 'lib/services/', 'COLLEGATO'),
  Motore('Cielo del momento', 'buildSkyFor', 'lib/core/astro/sky.dart',
      'COLLEGATO'),
  Motore(
      'Segno solare', 'sunSign', 'lib/core/astro/night_sky.dart', 'COLLEGATO'),
  Motore(
      'Segno lunare', 'moonSign', 'lib/core/astro/night_sky.dart', 'COLLEGATO'),
  Motore('Fase lunare', 'MoonPhase.forDate', 'lib/core/astro/moon_phase.dart',
      'COLLEGATO'),
  Motore('Illuminazione lunare', 'moonIllumination',
      'lib/core/astro/celestial.dart', 'COLLEGATO'),
  Motore('Risonanza', 'computeResonance', 'lib/core/astro/resonance.dart',
      'COLLEGATO'),
  Motore('Numero della vita', 'lifePathNumber',
      'lib/core/identity/numerology.dart', 'COLLEGATO'),
  Motore('Animale Guida', 'GuideAnimalDerivation',
      'lib/core/rituals/guide_animal_derivation.dart', 'COLLEGATO'),
  Motore('Animale del giorno', 'GuideAnimalDay',
      'lib/core/rituals/guide_animal_day.dart', 'SEGNAPOSTO DICHIARATO'),
  Motore('Angeli custodi', 'GuardianAngels',
      'lib/core/angels/guardian_angels.dart', 'COLLEGATO'),
  Motore(
      'Test Archetipo', 'ArchetypeQuiz', 'lib/core/archetypes/', 'COLLEGATO'),
  Motore('Costellazione del Viso', 'FaceClassifier', 'lib/core/face/',
      'COLLEGATO'),
  Motore('Tarocchi, stesa', 'TarotSpread', 'lib/core/tarot/', 'COLLEGATO'),
  Motore('Tarocchi, lettura', 'TarotReading', 'lib/core/tarot/', 'COLLEGATO'),
  Motore(
      'Oroscopo', 'Horoscope', 'lib/core/horoscope/', 'SEGNAPOSTO DICHIARATO'),
  Motore('Doni del giorno', 'DawnGift', 'lib/core/rituals/',
      'SEGNAPOSTO DICHIARATO'),
  Motore('Rune, getto', 'GettataRune', 'lib/core/rituals/', 'COLLEGATO'),
  Motore(
      'Sigillo intenzione', 'IntentionSigil', 'lib/core/magic/', 'COLLEGATO'),
  Motore('Sinastria VIP', 'SynastryReport', 'lib/core/synastry/', 'COLLEGATO'),
  Motore(
      'Limiti dei piani', 'PlanCatalog', 'lib/core/entitlement/', 'COLLEGATO'),
  Motore('Identita di nascita', 'BirthIdentity',
      'lib/core/identity/birth_identity.dart', 'COLLEGATO'),
  Motore('Fatti natali', 'NatalFacts', 'lib/core/identity/natal_identity.dart',
      'COLLEGATO'),
  Motore('Luna di nascita', 'BirthMoon', 'lib/core/identity/birth_moon.dart',
      'COLLEGATO'),
];
