...
void main() {
  test('OGNI MAESTRO HA LA SUA FINESTRA, COL BORDO BASSO SUL TAGLIO', () {
    ...
    for (final m in Maestro.values) {
      final i = InquadraturaDelVolto.di(m);
      final r = i.ritaglio;
      // ignore: avoid_print
      print('EG.05 ${m.name}: ritaglio ${r.left.toStringAsFixed(3)} '
          '${r.top.toStringAsFixed(3)} ${r.right.toStringAsFixed(3)} '
          '${r.bottom.toStringAsFixed(3)}');
      expect(r.bottom, closeTo(i.basso, 1e-9),
          reason: '${m.name}: il bordo basso della finestra non e\' il taglio '
      ...
      expect(r.width / r.height,
          closeTo(InquadraturaDelVolto.proporzione, 1e-9),
          reason: '${m.name}: la finestra non ha le proporzioni della cornice');
      ...
      for (final zona in [i.toppa, i.intatto]) {
        ...
        expect(
            ...
            isTrue,
            reason: '${m.name}: la zona $zona esce dalla finestra $r');
      }
    }
    // Le due zone dichiarate, contate: senza, la prova sopra non guarda niente.
    final zone = [
      for (final m in Maestro.values) ...[
        InquadraturaDelVolto.di(m).toppa,
        InquadraturaDelVolto.di(m).intatto,
      ]
    ].whereType<Rect>().length;
    cardinaleMinimo(zone, 2,
        cosa: 'zone dichiarate nelle finestre',
  ...
  test('IL SILENZIO CHIUDE A TRENTA SECONDI, E SOLO QUANDO SI E\' VIVI', () {
    ...
    expect(
        vivo.con(fine: ComeFinisce.silenzio).laFraseDellaFine(),
