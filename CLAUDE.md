...
- Runtime AI: Vertex AI e Gemini (`gemini-2.5-flash` per la chat dei Maestri, per la risposta Profonda e per i task ripetitivi come il distillato di memoria; `gemini-2.5-flash-lite` per la risposta Breve, per il piano Free e per il presagio delle rune. **La regola della profondita' sta in un punto solo**, `FirebaseMaestroAiProvider.modelForDepth`: Profonda vuol dire Flash, tutto il resto Flash-Lite. Scelta di risparmio di Mauro del 21 settembre 2026, *"restiamo sul risparmio con flash"*). Voce: Gemini-TTS per il live, Chirp 3 HD per il batch. I nomi esatti dei modelli vanno riverificati allo sviluppo.
