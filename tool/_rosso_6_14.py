...
INNESTI = [
    ('la Luna torna al nome grezzo, e la frase si sgrammatica',
     VOCE,
     "        '${MoonPhase.comeSiDice(luna.italianName)}.';",
     "        '${luna.italianName.toLowerCase()}.';",
     lambda t: 'luna.italianName.toLowerCase()' in t.split('final cielo')[1][:200]),
    ('la porta che sa dire la Luna smette di saperlo',
     LUNA,
     A + "Ultimo quarto" + A + " => " + Q + "all" + A + "ultimo quarto" + Q + ",",
     A + "Ultimo quarto" + A + " => " + A + "ultimo quarto" + A + ",",
     lambda t: "'Ultimo quarto' => 'ultimo quarto'," in t),
    ('l eco della domanda torna su tutte e tre le rune',
     VOCE,
     "    final eco = pulita.isEmpty || indice != 0",
     "    final eco = pulita.isEmpty",
     lambda t: 'indice != 0' not in t.split('final eco')[1][:80]),
    ('il Sigillo torna a dire solo cosa e',
     SCHERMO,
     "                    SigilloDelGiorno.riga(",
     "                    'Le rune di questa gettata.' + String.fromCharCode(0) + (",
     lambda t: 'String.fromCharCode(0)' in t),
    ('la strofa torna a farsi leggere piu piccola',
     SCHERMO,
     "                  style: TypographyTokens.lettura()" + NL +
     "                      .copyWith(color: ColorTokens.textSecondary)),",
     "                  style: TypographyTokens.corpo()" + NL +
     "                      .copyWith(color: ColorTokens.textSecondary)),",
