# Testi della Sinastria VIP, revisione B

Scritti dall'Architetto il 28 agosto 2026, su richiesta del fondatore, per
sostituire integralmente il corpus precedente `docs/corpus/sinastria_testi.md`.

## Perché questa revisione esiste

Il fondatore ha giudicato il responso attuale scarno, e ha ragione: quello che
la persona legge oggi è un montaggio di tre frammenti che non si parlano fra
loro. Nella lettura con DiCaprio, su otto righe tre sono occupate da una nota
tecnica sull'ora di nascita, una è un fatto astronomico espresso in gradi, una
è una frase sul personaggio che arriva senza cucitura, e la chiusura è la
stessa per tutti. La frase dentro il cerchio dipende soltanto dalla fascia di
percentuale, e per questo si ripete sempre uguale.

Questo testo deve fare un mestiere diverso: **deve essere condiviso**. Il
fondatore lo ha chiesto memorabile, malizioso, d'impatto e un po' esagerato.

## La regola che governa ogni riga di questo corpus

**Nessun testo dell'app afferma qualcosa sulla vita privata di una persona
reale.** Niente sentimenti, relazioni, separazioni, salute, denaro personale,
guai giudiziari, niente "pare che" e niente insinuazioni.

L'esagerazione vive su due cose che non si possono querelare: **la relazione
fra i due segni**, e **ciò che il personaggio fa in pubblico come
professionista**. La differenza è netta: "ha vinto Wimbledon a luglio" è
cronaca, "dopo la crisi con" è gossip. Il primo si può dire, il secondo no.

Questa regola vale anche per l'attualità aggiornata: il campo che la porta
contiene un fatto pubblico, verificabile e datato, mai una notizia di
costume.

**Il tono può essere sfacciato quanto vuole, purché il bersaglio della
battuta siano i due cieli e non la vita di nessuno.** È anche la scelta più
efficace: chi legge condivide una frase che parla di sé, non di una
celebrità.

## Come si compone un responso

Il responso è fatto di cinque pezzi, e ognuno ha una casa precisa a schermo.

1. **La frase sopra il cerchio.** Una riga sola, la prima cosa che si legge.
   Nasce dalla relazione fra i due segni e dalla fascia di affinità, quindi
   cambia davvero. Sostituisce l'etichetta fissa dentro il cerchio.
2. **Il titolo della bolla.** Poche parole, memorabili, un po' esagerate.
   Nasce dalla stessa coppia relazione più fascia.
3. **Il corpo della bolla.** Quattro frasi cucite: l'apertura sui due segni,
   il cielo reso leggibile, il personaggio con la sua attualità, la stoccata
   finale.
4. **La nota, fuori dalla bolla.** Tutto ciò che oggi appesantisce il testo
   virale (l'ora di nascita che manca, la distanza sconosciuta) esce dalla
   bolla e vive sotto, in corpo minore, dove è giusto che stia.
5. **La sfida da condividere.** Una riga sotto il pulsante, già esistente,
   che qui viene resa variabile.

## Le sette relazioni fra i segni

Restano quelle del corpus precedente, perché sono corrette e coprono tutti i
casi. Elementi: Fuoco Ariete Leone Sagittario, Terra Toro Vergine Capricorno,
Aria Gemelli Bilancia Acquario, Acqua Cancro Scorpione Pesci.

- **stesso**: stesso segno
- **elemento**: stesso elemento, segni diversi
- **fuocoaria**: fuoco con aria
- **terracqua**: terra con acqua
- **opposti**: segni a sei di distanza
- **tensione**: elementi che si sfidano
- **estranei**: ogni altro caso

## Le cinque fasce di affinità

- **leggendaria**: da 90 in su
- **alta**: da 75 a 89
- **buona**: da 60 a 74
- **curiosa**: da 45 a 59
- **attrito**: sotto 45

## 1. LE FRASI SOPRA IL CERCHIO

Trentacinque righe, una per ogni incrocio di relazione e fascia. Una riga
sola, breve, che si legge prima della percentuale.

### stesso segno

- leggendaria: "Siete la stessa persona in due corpi. Che paura."
- alta: "Stesso segno, stessi difetti, stessa faccia tosta."
- buona: "Vi capireste anche al buio e infatti spesso lo fate."
- curiosa: "Uguali sì, ma non è detto che sia un complimento."
- attrito: "Due copie dello stesso originale non fanno una coppia."

### stesso elemento

- leggendaria: "Stessa materia, stessa fiamma, stessa follia."
- alta: "Parlate la stessa lingua senza aver mai preso lezioni."
- buona: "Vi riconoscete a distanza, come fa la gente della stessa razza."
- curiosa: "Stessa sostanza, forme diverse: vi somigliate a giorni alterni."
- attrito: "Anche l'acqua e il ghiaccio sono la stessa cosa e non vanno d'accordo."

### fuoco e aria

- leggendaria: "Uno accende, l'altra soffia. È così che nascono gli incendi."
- alta: "Insieme fareste saltare la corrente a mezzo quartiere."
- buona: "Vi date fuoco a vicenda, ma nel senso bello."
- curiosa: "C'è aria per bruciare, manca solo qualcuno che accenda."
- attrito: "Troppo vento spegne anche il fuoco più deciso."

### terra e acqua

- leggendaria: "Una terra così e un'acqua così fanno nascere qualsiasi cosa."
- alta: "Uno tiene, l'altra nutre: si chiama fortuna."
- buona: "Un'intesa che non fa rumore e regge il peso."
- curiosa: "Terreno buono, pioggia incerta: vediamo che raccolto viene."
- attrito: "Troppa acqua su troppa terra fa solo fango."

### segni opposti

- leggendaria: "Agli antipodi dello zodiaco e non riuscite a smettere di guardarvi."
- alta: "Il contrario esatto di te ti sta guardando. Buona fortuna."
- buona: "L'attrazione dei contrari, quella che nessuno sa spiegare."
- curiosa: "Opposti abbastanza da incuriosirvi, non abbastanza da capirvi."
- attrito: "Vi guardate dai due capi del tavolo e il tavolo è lungo."

### elementi in tensione

- leggendaria: "Una tensione così alta di solito finisce sui giornali."
- alta: "Chimica pura e nessuna via d'uscita: complimenti."
- buona: "Vi sfidate e la cosa vi diverte più di quanto ammettiate."
- curiosa: "Scintille sì, incendio non pervenuto."
- attrito: "Due elementi che si annullano a vicenda, con garbo."

### estranei

- leggendaria: "Non avete niente in comune e vi trovate benissimo. Spiegatelo voi."
- alta: "Due mondi diversi che hanno deciso di piacersi lo stesso."
- buona: "Curiosi l'uno dell'altro, che è già più di quanto capiti di solito."
- curiosa: "Vi guardate come si guarda una lingua straniera."
- attrito: "Due mondi diversi e i visti non si sono trovati."

## 2. I TITOLI DELLA BOLLA

Trentacinque titoli, stessa griglia. Poche parole, memorabili, da titolo di
giornale.

### stesso segno

- leggendaria: "Lo specchio perfetto"
- alta: "Due gocce d'acqua e una tempesta"
- buona: "Conosco già tutti i tuoi trucchi"
- curiosa: "Troppo simili per stupirsi"
- attrito: "Il difetto in doppia copia"

### stesso elemento

- leggendaria: "Stessa materia, stesso destino"
- alta: "Nati dalla stessa scintilla"
- buona: "Ci si riconosce tra simili"
- curiosa: "Parenti alla lontana"
- attrito: "Stessa origine, strade opposte"

### fuoco e aria

- leggendaria: "L'incendio annunciato"
- alta: "Benzina e fiammiferi"
- buona: "Chi accende e chi soffia"
- curiosa: "Manca solo la scintilla"
- attrito: "Troppo vento per una fiamma sola"

### terra e acqua

- leggendaria: "Tutto quello che tocca fiorisce"
- alta: "Radici e pioggia"
- buona: "L'intesa che non fa rumore"
- curiosa: "Un raccolto da vedere"
- attrito: "Fango, per ora"

### segni opposti

- leggendaria: "Il disastro perfetto"
- alta: "Il tuo contrario esatto"
- buona: "L'attrazione che nessuno spiega"
- curiosa: "Vicini di zodiaco, lontani di testa"
- attrito: "Ai due capi del tavolo"

### elementi in tensione

- leggendaria: "Roba da prima pagina"
- alta: "Chimica pericolosa"
- buona: "La sfida che vi diverte"
- curiosa: "Scintille senza incendio"
- attrito: "Si annullano con eleganza"

### estranei

- leggendaria: "Nessuno lo capisce, funziona"
- alta: "Due mondi che hanno deciso"
- buona: "Curiosità reciproca"
- curiosa: "Una lingua straniera"
- attrito: "Visti non concessi"

## 3. LE APERTURE DEL CORPO

La prima frase della bolla. Sostituisce "Il fatto è questo", che oggi apre
ogni responso allo stesso modo. Ce ne sono tre per relazione, scelte a
rotazione senza ripetere la stessa due volte di fila. Nel testo, SEGNO_A e
SEGNO_B sono i due segni, NOME è il personaggio.

### stesso

- "Due SEGNO_A nella stessa stanza sono uno spettacolo o un disastro e non c'è mai una terza possibilità."
- "Sei un SEGNO_A e hai davanti un altro SEGNO_A: sai già come va a finire ed è questo il problema."
- "Stesso segno vuol dire stesse virtù, ma soprattutto stessi vizi e i vizi in doppia copia si notano."

### elemento

- "SEGNO_A e SEGNO_B sono fatti della stessa materia e la materia si riconosce da lontano."
- "Stesso elemento, forme diverse: vi capite prima di parlare, il che vi risparmia parecchie discussioni."
- "Fra un SEGNO_A e un SEGNO_B non serve spiegarsi: serve semmai decidere chi comanda."

### fuocoaria

- "Il fuoco di SEGNO_A e l'aria di SEGNO_B: uno accende, l'altra alimenta e nessuno dei due sa dove finisce."
- "Mettere insieme SEGNO_A e SEGNO_B è come lasciare una finestra aperta accanto a un camino acceso."
- "SEGNO_A brucia, SEGNO_B soffia: la fisica è dalla vostra parte, la prudenza molto meno."

### terracqua

- "La terra di SEGNO_A e l'acqua di SEGNO_B fanno la cosa più rara dello zodiaco: qualcosa che dura."
- "SEGNO_A tiene, SEGNO_B nutre: non è romantico da raccontare, ma è quello che regge quando il resto crolla."
- "Fra SEGNO_A e SEGNO_B non ci sono fuochi d'artificio, c'è un raccolto."

### opposti

- "SEGNO_A e SEGNO_B stanno ai due capi opposti del cerchio, il che significa che avete lo stesso identico problema visto da due lati."
- "Un SEGNO_A e un SEGNO_B sono la stessa medaglia girata: per questo vi attraete e per questo vi date sui nervi."
- "Nello zodiaco SEGNO_A e SEGNO_B si guardano da lontano e non riescono a smettere."

### tensione

- "SEGNO_A e SEGNO_B appartengono a elementi che non si sono mai piaciuti e le storie migliori nascono così."
- "Fra SEGNO_A e SEGNO_B c'è attrito e l'attrito è l'unica cosa che produce calore."
- "SEGNO_A e SEGNO_B si sfidano per natura: la domanda non è se litigherete, è quanto vi divertirete."

### estranei

- "SEGNO_A e SEGNO_B non hanno quasi niente in comune e questo li rende reciprocamente inspiegabili."
- "Fra un SEGNO_A e un SEGNO_B non c'è una lingua comune: si comincia imparandola."
- "SEGNO_A e SEGNO_B vengono da due mondi che non si erano mai parlati prima di adesso."

## 4. IL CIELO RESO LEGGIBILE

Oggi il responso dice "il suo Sole in opposizione alla tua Venere, a 0,0 gradi
dall'angolo esatto". È vero, ed è il pezzo più prezioso che l'app possiede,
perché nessuna app di gossip ce l'ha. Ma detto così è un referto.

La regola: **prima cosa significa, poi come si chiama.** Il nome tecnico
resta, perché è la prova che il numero non è inventato, ma viene dopo.

Per ogni combinazione di pianeta e aspetto esiste una riga di significato.
Sotto, le combinazioni che il motore produce più spesso, con il loro
significato in una riga. La struttura è: SIGNIFICATO, poi il nome tecnico fra
parentesi.

### Con Venere

- Venere congiunzione Venere: "Vi piacciono le stesse cose, il che è comodo e pericoloso insieme."
- Venere opposizione Venere: "Vi piacciono cose opposte e nessuno dei due ha intenzione di cedere."
- Venere trigono Venere: "Avete lo stesso gusto senza esservi messi d'accordo."
- Venere sestile Mercurio: "Uno parla e l'altro trova affascinante il modo in cui lo dice."
- Sole opposizione Venere: "Lui è esattamente il tipo che non dovresti trovare interessante."
- Venere quadrato Marte: "Vi attirate e vi irritate nello stesso istante ed è quello il punto."

### Con la Luna

- Luna congiunzione Luna: "Reagite alle cose nello stesso modo, spesso nello stesso momento."
- Marte sestile Luna: "Uno si muove e l'altro lo sente prima che accada."
- Luna opposizione Sole: "Uno illumina, l'altro assorbe: funziona finché nessuno pretende il contrario."
- Luna quadrato Mercurio: "Uno sente, l'altro spiega e non si trovano mai sulla stessa frase."

### Con Marte

- Marte congiunzione Marte: "Stessa energia, stessa fretta, stesso muro contro cui andare."
- Marte sestile Ascendente: "Uno entra in una stanza e l'altro se ne accorge subito."
- Marte trigono Sole: "Uno spinge e l'altro non si tira indietro."

### Con Mercurio

- Mercurio congiunzione Sole: "Pensa a voce alta e voi lo capite al primo tentativo."
- Mercurio trigono Mercurio: "Due teste che corrono alla stessa velocità e si annoiano insieme del resto del mondo."
- Mercurio quadrato Saturno: "Uno propone, l'altro obietta e alla fine hanno ragione entrambi."

### La regola per le combinazioni non elencate

Quando il motore produce una combinazione che non ha una riga sua, si usa la
riga generica del pianeta più lento fra i due, e non si inventa nulla:

- Sole: "C'è qualcosa nel suo modo di stare al mondo che non ti lascia indifferente."
- Luna: "Vi toccate un nervo scoperto e non sempre nello stesso modo."
- Mercurio: "Il modo in cui pensate si incrocia più di quanto sembri."
- Venere: "Sul bello e sul piacere avete parecchio da dirvi."
- Marte: "Quando si tratta di agire, vi accorgete l'uno dell'altro."
- Saturno: "C'è un peso in mezzo e i pesi tengono insieme più di quanto separino."

## 5. I CINQUANTA PERSONAGGI

Ogni personaggio ha ora **due** righe invece di una.

La **presentazione** è la sua carta d'identità pubblica, e non scade mai.
Rispetto al corpus precedente è scritta per stare dentro una frase, non per
essere appiccicata in mezzo.

L'**attualità** è il campo nuovo, e va nel catalogo `docs/corpus/vip.json`
sotto la chiave `attualita`, con accanto la data in cui è stata verificata.
Contiene **un solo fatto pubblico e professionale**, breve. Qui sotto è
scritto in corsivo il tipo di fatto che ci va, non il fatto stesso, perché
quello lo scrive chi aggiorna il catalogo.

Quando il campo è vuoto o più vecchio di novanta giorni, il testo lo salta e
non se ne accorge nessuno: la frase regge lo stesso.

- **vip_angelina-jolie**: divide la vita fra un set e mezzo mondo da salvare. *Attualità: film, regia, missioni ufficiali.*
- **vip_ariana-grande**: arriva a note che il resto dell'umanità nemmeno immagina. *Attualità: album, tour, film.*
- **vip_bad-bunny**: riempie gli stadi cantando in spagnolo e non ha mai chiesto permesso. *Attualità: album, tour, premi.*
- **vip_beyonce**: quando entra lei, le altre luci si spengono da sole. *Attualità: album, tour, premi.*
- **vip_bill-gates**: ha riscritto il mondo partendo da un garage e adesso prova a ripararlo. *Attualità: iniziative pubbliche, libri.*
- **vip_billie-eilish**: sussurra e la ascoltano in milioni. *Attualità: album, colonne sonore, premi.*
- **vip_brad-pitt**: invecchia meglio del vino e lo sa benissimo. *Attualità: film, produzioni.*
- **vip_chiara-ferragni**: di un post ha saputo fare un impero. *Attualità: progetti imprenditoriali pubblici.*
- **vip_damiano-david**: sul palco perde la camicia ma mai il ritmo. *Attualità: album, tour.*
- **vip_dicaprio**: colleziona Oscar e tramonti, in quest'ordine. *Attualità: film, iniziative ambientali.*
- **vip_drake**: trasforma ogni dispiacere in un disco di platino. *Attualità: album, primati di ascolti.*
- **vip_dwayne-johnson**: solleva più peso del tuo intero condominio. *Attualità: film, imprese.*
- **vip_elon-musk**: twitta a mezzanotte e sposta i mercati. *Attualità: lanci, aziende, annunci pubblici.*
- **vip_emma-watson**: dai libri di magia è passata a quelli veri. *Attualità: film, studi, cause pubbliche.*
- **vip_federer**: perdeva con eleganza anche quando vinceva. *Attualità: apparizioni, iniziative.*
- **vip_fedez**: fa notizia più di un telegiornale e con meno sforzo. *Attualità: musica, progetti pubblici.*
- **vip_giorgio-armani**: ha vestito il mondo di grigio elegante. *Attualità: sfilate, riconoscimenti.*
- **vip_jeff-bezos**: ti consegna tutto tranne il suo tempo libero. *Attualità: aziende, lanci spaziali.*
- **vip_kanye-west**: una ne fa e cento ne pensa e le pensa ad alta voce. *Attualità: album, progetti.*
- **vip_keanu-reeves**: resta gentile pure mentre salva il mondo. *Attualità: film, progetti.*
- **vip_kim-kardashian**: della propria vita ha fatto un impero quotato. *Attualità: imprese, studi legali.*
- **vip_kylie-jenner**: a vent'anni contava già i miliardi. *Attualità: imprese, lanci.*
- **vip_lady-gaga**: cambia faccia a ogni canzone ma non cambia mai voce. *Attualità: album, tour, film.*
- **vip_lebron-james**: a quarant'anni vola ancora e la cosa comincia a innervosire. *Attualità: stagione, primati.*
- **vip_margot-robbie**: ha reso una bambola un fenomeno mondiale. *Attualità: film, produzioni.*
- **vip_mark-zuckerberg**: sa tutto di te ma non risponde ai messaggi. *Attualità: prodotti, annunci.*
- **vip_mbappe**: corre più veloce del tuo wifi. *Attualità: stagione, trofei.*
- **vip_messi**: parla poco e segna sempre, che è il modo migliore di rispondere. *Attualità: stagione, trofei.*
- **vip_michelle-obama**: ha rimesso di moda l'intelligenza. *Attualità: libri, interventi pubblici.*
- **vip_monica-bellucci**: il tempo lo guarda passare senza farsi toccare. *Attualità: film, teatro.*
- **vip_nadal**: non molla un punto nemmeno per sbaglio. *Attualità: apparizioni, accademia.*
- **vip_oprah-winfrey**: regala macchine e cambia vite, spesso nello stesso pomeriggio. *Attualità: interviste, progetti.*
- **vip_priyanka-chopra**: ha conquistato due continenti senza chiedere il permesso a nessuno. *Attualità: film, serie, imprese.*
- **vip_rihanna**: tra un disco e l'altro ti ha pure venduto il fondotinta. *Attualità: musica, imprese.*
- **vip_ronaldo**: si allena mentre tu dormi e te lo fa sapere. *Attualità: stagione, primati.*
- **vip_scarlett-johansson**: ha dato la voce persino ai robot. *Attualità: film, regia.*
- **vip_selena-gomez**: sopravvive a Hollywood col sorriso, che è la cosa più difficile. *Attualità: musica, serie, imprese.*
- **vip_serena-williams**: serve più forte di quanto tu discuta. *Attualità: imprese, apparizioni.*
- **vip_shakira**: con i fianchi non sa mentire, con il resto nemmeno. *Attualità: album, tour.*
- **vip_sinner**: resta di ghiaccio anche quando il campo prende fuoco. *Attualità: tornei, classifica.*
- **vip_snoop-dogg**: se la prende comoda da trent'anni e non ha mai avuto fretta. *Attualità: musica, apparizioni.*
- **vip_steve-jobs**: ha messo il futuro nella tasca di tutti. *Attualità: nessuna e il testo lo dichiara.*
- **vip_taylor-swift**: se la lasci ci scrive un album e vende tutto. *Attualità: album, tour, primati.*
- **vip_the-weeknd**: canta le notti che tu dimentichi. *Attualità: album, tour.*
- **vip_timothee-chalamet**: fa sospirare due generazioni contemporaneamente. *Attualità: film, premi.*
- **vip_tom-cruise**: gli stunt se li fa da solo e a una certa età è quasi un dispetto. *Attualità: film, imprese.*
- **vip_usain-bolt**: ha corso piano solo per salutare. *Attualità: apparizioni, imprese.*
- **vip_valentino-rossi**: in curva piega più di te sotto le scadenze. *Attualità: gare, squadra.*
- **vip_warren-buffett**: a colazione compra aziende. *Attualità: operazioni, lettera annuale.*
- **vip_zendaya**: a ogni tappeto rosso manda in tilt internet. *Attualità: film, serie, moda.*

## 6. COME ENTRA L'ATTUALITÀ NEL TESTO

L'attualità non è una frase a sé: si innesta nella presentazione con una di
queste giunture, scelte a rotazione. NOME è il personaggio, FATTO è il
contenuto del campo.

- "NOME, che PRESENTAZIONE e che in questi mesi FATTO."
- "NOME PRESENTAZIONE. Di questi tempi, poi, FATTO."
- "Aggiungi che NOME FATTO e capisci con chi hai a che fare."

Quando il campo manca, è vecchio o il personaggio non è più in vita, si usa
la sola presentazione, e nessuno si accorge che manchi qualcosa.

**Per chi non è più in vita** il testo cambia tempo verbale e non fa mai
dell'ironia sulla morte. Si usa questa forma: "NOME, che PRESENTAZIONE."
Al passato, e la stoccata finale viene sostituita da una riga sobria, che sta
nell'elenco delle chiusure sotto la voce memoria.

## 7. LE STOCCATE FINALI

L'ultima frase della bolla. Sostituisce le quattro chiusure precedenti, che
erano tutte sull'improbabilità dell'incontro e si ripetevano. Ce ne sono
cinque per fascia, scelte a rotazione senza ripetere la stessa due volte di
fila.

### leggendaria

- "Un cielo così non capita a tutti. Fanne buon uso, o almeno raccontalo bene."
- "Se credi ai numeri, questo è di quelli che non si spiegano."
- "Percentuali così alte di solito le vede solo chi bara."
- "Il cosmo ha fatto la sua parte. Il resto, come sempre, è un altro discorso."
- "Tienitelo stretto questo risultato: non ne arrivano molti."

### alta

- "Il cielo dice di sì con una certa convinzione."
- "Su carta funzionereste benissimo. La carta, si sa, sopporta tutto."
- "Un'intesa così andrebbe sprecata a non usarla."
- "Le stelle hanno votato compatte, per una volta."
- "C'è più di quanto ti aspettassi, ammettilo."

### buona

- "Non male, considerando che uno dei due non sa nemmeno che esisti."
- "Ci sono le basi. Manca solo tutto il resto."
- "Il cielo dice che potreste andare d'accordo. Il cielo è ottimista di natura."
- "Abbastanza per una bella serata, forse anche per due."
- "Un buon risultato, di quelli che non si vantano ma tengono."

### curiosa

- "Il cielo non si sbilancia e fa bene."
- "Né un sì né un no: il cosmo prende tempo."
- "Abbastanza per incuriosirsi, non abbastanza per fare progetti."
- "Una di quelle percentuali che dipendono più da voi che dalle stelle."
- "Il cielo lascia la porta socchiusa e vediamo chi la spinge."

### attrito

- "Il cielo è stato chiaro e non è colpa nostra."
- "Su questo, francamente, le stelle hanno delle riserve."
- "Poche percentuali, ma le storie migliori nascono così, dicono."
- "Il cosmo dice di no. Il cosmo però ha sbagliato altre volte."
- "Numeri bassi, ma nessuno ha mai chiesto il permesso allo zodiaco."

### memoria, per chi non è più in vita

- "Un cielo che continua a dire la sua, anche adesso."
- "I cieli restano, anche quando le persone se ne vanno."
- "Certe carte del cielo non smettono di parlare."

## 8. LA NOTA, FUORI DALLA BOLLA

Tutto ciò che oggi appesantisce il testo virale esce dalla bolla e vive
sotto, in corpo minore. Serve, perché è la regola di trasparenza del
progetto, ma non deve stare in mezzo alla battuta.

- ora di nascita ignota: "Del suo cielo non si conosce l'ora esatta di nascita: questa lettura guarda i pianeti, non l'Ascendente. Non si finge di sapere ciò che nessuna fonte dichiara."
- luogo ignoto: "Dove viva non è cosa pubblica, quindi la distanza non entra nel conto."
- attualità non aggiornata: "Le notizie su questa persona sono aggiornate al GIORNO."

## 9. LA SFIDA DA CONDIVIDERE

La riga sopra il pulsante di condivisione, oggi sempre uguale. Cinque
varianti a rotazione. NOME è il personaggio, PERCENTO il risultato.

- "E tu con NOME quanto fai? Sfida i tuoi amici."
- "PERCENTO con NOME. Scommetti che il tuo amico fa peggio?"
- "Manda questa a chi giura di essere l'anima gemella di NOME."
- "Chi dei tuoi amici batte PERCENTO con NOME? Scoprilo."
- "NOME e te, PERCENTO. Vediamo chi fa meglio nel tuo giro."

## 10. TRE ESEMPI COMPOSTI PER INTERO

### Utente Gemelli con DiCaprio, Scorpione, 54 per cento

Sopra il cerchio: "Vi guardate come si guarda una lingua straniera."

Titolo della bolla: "Una lingua straniera"

Corpo: "Gemelli e Scorpione non hanno quasi niente in comune, e questo li
rende reciprocamente inspiegabili. Lui è esattamente il tipo che non dovresti
trovare interessante, e il cielo lo scrive chiaro: il suo Sole in opposizione
alla tua Venere, esatto al grado. Leonardo DiCaprio, che colleziona Oscar e
tramonti in quest'ordine, e che in questi mesi FATTO. Il cielo non si
sbilancia, e fa bene."

Nota sotto: "Del suo cielo non si conosce l'ora esatta di nascita: questa
lettura guarda i pianeti, non l'Ascendente."

Sfida: "54 per cento con Leonardo DiCaprio. Scommetti che il tuo amico fa
peggio?"

### Bad Bunny, Pesci, con Lady Gaga, Ariete, 67 per cento

Sopra il cerchio: "Vi guardate come si guarda una lingua straniera."

Titolo della bolla: "Curiosità reciproca"

Corpo: "Pesci e Ariete vengono da due mondi che non si erano mai parlati
prima di adesso. Uno parla e l'altra trova affascinante il modo in cui lo
dice: il suo Mercurio in sestile alla sua Venere, esatto al grado. Da una
parte Bad Bunny, che riempie gli stadi cantando in spagnolo e non ha mai
chiesto permesso; dall'altra Lady Gaga, che cambia faccia a ogni canzone ma
non cambia mai voce. Non male, considerando che nessuno dei due sa di essere
finito in questa classifica."

### Utente Leone con Zendaya, Vergine, 91 per cento

Sopra il cerchio: "Non avete niente in comune e vi trovate benissimo. Spiegatelo voi."

Titolo della bolla: "Nessuno lo capisce, funziona"

Corpo: "Leone e Vergine non hanno quasi niente in comune, e questo li rende
reciprocamente inspiegabili. Eppure vi piacciono le stesse cose, il che è
comodo e pericoloso insieme: la sua Venere in trigono alla tua, esatta al
grado. Zendaya, che a ogni tappeto rosso manda in tilt internet, e che in
questi mesi FATTO. Percentuali così alte di solito le vede solo chi bara."

## 11. COSA SERVE PERCHÉ QUESTO CORPUS FUNZIONI

Tre cose, e sono lavoro di Code, non dell'Architetto.

1. Il campo `attualita` e il campo `attualita_verificata_il` nel catalogo dei
   personaggi, con la regola dei novanta giorni.
2. La frase sopra il cerchio, il titolo della bolla e la nota separata come
   pezzi distinti a schermo, perché oggi non esistono.
3. La scelta a rotazione senza ripetizione immediata su aperture, stoccate e
   sfide, come già avviene per le chiusure attuali.
