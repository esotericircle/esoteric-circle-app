...
- Funzioni live nel function_shelf: Sinastria VIP, Oracolo del Giorno, Runa del Tramonto, Meditazione. Non ancora live: Stesa a Tre Carte, Test Archetipo, Costellazione del Viso. La Sinastria VIP e' stata rifatta a video: cosmo profondo di sfondo, i due volti nella cornice ad arco col cuore, cerchio grande con percentuale animata ed etichetta di fascia, responso composto dal Corpus `docs/corpus/sinastria_testi.md` (relazione tra i segni piu' carattere del VIP per stem piu' chiusura ironica) prima delle barre, quattro barre deterministiche per coppia (amore, intesa mentale, scintille, incontro volutamente minimo 0,2..4 per cento con micro battuta), il cerchio come sintesi pesata delle prime tre. Foto utente opzionale con consenso esplicito a fotocamera o galleria (image_picker, stringhe permesso in Info.plist), tenuta solo in memoria, filtro dorato e cornice ad arco ricostruita a vettori (`vip_arch_frame.dart`), mai caricata. Tasto Condividi che genera la card oro e blu di Medora come PNG e apre il foglio social (share_plus). Logica in `lib/core/synastry/synastry_report.dart`, card in `lib/features/synastry/sinastria_share_card.dart`. La vecchia classe `Synastry` per elemento e' stata sostituita da `SynastryReport`. La fonte macchina di questo stato e' `docs/stato_funzioni.json`, allineata al codice da `test/stato_funzioni_test.dart` nel verde della CI: se il manifest e `lib/core/santuario/function_shelf.dart` divergono, la spunta verde cade.
