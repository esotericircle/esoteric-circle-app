...
- **BQ.01** I tre video entrano nel repository. **CHIUSA il 27 agosto 2026, ordine BT voce 03.** I tre file sono arrivati il 26 agosto, entrano nel pacchetto e viaggiano dentro la build 2207 consegnata la notte del 27. **I PESI SONO MISURATI, uno per uno, dal disco di questo PC**, non presi dall'ordine: `medora_rivelazione.mp4` **2.422.069 byte**, `caligo_rivelazione.mp4` **1.995.875**, `aura_rivelazione.mp4` **2.831.474**, in tutto **7.249.418 byte**. Misurati anche il resto, con `ffprobe`: tutti e tre **H.264 profilo High, pixel `yuv420p` a 8 bit, 24 fotogrammi al secondo, 241 fotogrammi, 10,041667 secondi, nessuna traccia audio**; Medora e Caligo **720x1280**, Aura **1080x1920**, quindi 9 a 16 esatto per tutti e tre; le velocita' sono 1.929.615, 1.590.074 e 2.255.780 bit al secondo. **Non sono i file dell'ordine**: gli originali su `D:` erano HEVC e Aura era HEVC Main 10 a 10 bit, e una copia fedele avrebbe messo nell'app tre filmati che una fetta larga di telefoni Android non decodifica. Restano **provvisori**, parole del fondatore: sono un test, due su tre portano il watermark e vanno ingranditi, tutti e tre vanno ottimizzati. **La storia di come questa voce si era fermata sta nel capitolo qui sotto, e non e' stata cancellata.**
...
- **BQ.04** La suite e il peso. CHIUSA per la parte misurabile, **e la parte non misurabile e' dichiarata invece di essere stimata**. `flutter analyze` sul progetto intero: **zero avvisi**. **IL PESO PRIMA**, contato sommando i file di tutte e ventinove le voci dichiarate fra gli asset del `pubspec.yaml`: **122.002.668 byte, cioe' 116,35 MiB**. I tre pesi maggiori sono il mazzo dei tarocchi con 25,07 MiB, gli Angeli con 23,82 e i ritratti VIP con 12,14; l'intro da sola pesa 5,87 MiB. **IL PESO DOPO E' MISURATO, ordine BT voce 03**, e la riga che stava qui diceva un conto invece di una misura: e' conservata nel capitolo che racconta come queste voci si erano fermate. Gli asset dichiarati pesano oggi **129.264.184 byte, cioe' 123,28 MiB**, contati come li conta Flutter, prendendo i file dentro ogni cartella dichiarata e non le sue sottocartelle; erano 122.002.668, quindi **piu' 5,95 per cento**. I tre filmati pesano **2.422.069**, **1.995.875** e **2.831.474** byte, in tutto **7.249.418**, letti dal disco uno per uno; la cartella `brand_assets/maestri/` ne porta **7.251.097** contando anche il suo LEGGIMI. **E il peso degli asset NON e' il peso dell'APK**: l'archivio comprime, e il peso vero si legge solo dall'archivio costruito. Quello della 2207 pesa **173.822.285 byte**. **UN GRAMMO DI ZAVORRA DICHIARATO**: `brand_assets/maestri/LEGGIMI.md` pesa 1.641 byte ed entra nel pacchetto, perche' e' dentro una cartella dichiarata; e' la stessa cosa che gia' succede a `assets/audio/LEGGIMI.md`, quindi la scelta e' coerente con cio' che il progetto fa gia', e il giorno che qualcuno vorra' togliere tutti i LEGGIMI dal pacchetto li togliera' insieme. **DUE GUARDIE DI CASA HANNO PRESO IL MIO CODICE NUOVO, e nessuna delle dodici prove che ho scritto io le guardava**: `stato_asset_test` ha visto che il `pubspec.yaml` dichiarava `brand_assets/maestri/` mentre `docs/stato_asset.json` non la conosceva, cioe' due elenchi di asset che si erano gia' separati; `nessun_catch_muto_test` ha visto il `catch (_)` del lettore vero e ha preteso che dichiarasse PERCHE' ignora l'errore. Adesso il manifest degli asset porta la cartella con la sua nota, e il catch nomina l'errore, lo scrive in console e spiega che sotto il velo c'e' gia' il ritratto, quindi non c'e' niente da salvare. **Le ha trovate la suite intera e non la rilettura**, ed e' il secondo ordine di fila in cui succede.
- **BQ.05** La build, ordinata dal fondatore. **CHIUSA il 27 agosto 2026, ordine BT voce 03.** La build esiste e la consegna e' avvenuta: versione **2207**, numero letto **dall'archivio** con `aapt2 dump badging` e non dal `pubspec`, costruita con `--target-platform android-arm64` e senza `--split-per-abi`, archivio **173.822.285 byte**, consegnata su App Distribution a `cloud@esotericircle.app` con release **`691vo1475ie70`**. **E contiene i tre video**, che era l'unica cosa che il fondatore aveva chiesto di controllare. **La storia di come questa voce si era fermata sta nel capitolo qui sotto, e non e' stata cancellata.**

## Come BQ.01 e BQ.05 si erano fermate, e come si sono chiuse

**QUESTO CAPITOLO ESISTE PERCHE' LA STORIA NON SI RISCRIVE.** Ordine BT voce 03.
Le due voci si erano fermate su una premessa che allora era vera, e il testo con
cui si erano fermate sta qui sotto **verbatim**: e' stato spostato, non
cancellato. Chi legge deve poter vedere che cosa mancava e perche', altrimenti
un manifesto tutto chiuso sembra un lavoro che non ha mai incontrato un muro.

**COSA E' SUCCESSO DOPO.** Il 26 agosto 2026 i tre file sono arrivati sul PC del
fondatore ed entrati nel repository con tre commit uno per uno, `ba04cc9f`,
`99f4b2c7` e `cb5ffb26`, ricodificati e non copiati per la ragione scritta nella
voce BQ.01. La build **2206** e' stata consegnata quello stesso giorno alle 14:26
UTC, release `4igu53cugnei8`, e la **2207** la notte del 27 con release
`691vo1475ie70`: tutte e due contengono i tre filmati. La premessa che teneva
ferme le due voci e' caduta, e le voci si chiudono.

### Il testo con cui BQ.01 si era fermata
...
### Il testo con cui BQ.05 si era fermata
...
**E IL PESO DOPO, che allora non si poteva misurare, adesso e' misurato.** Gli
asset dichiarati nel `pubspec.yaml`, contati come li conta Flutter, cioe'
prendendo i file dentro ogni cartella dichiarata e non le sue sottocartelle,
pesano **129.264.184 byte, cioe' 123,28 MiB**, contro i 122.002.668 misurati
prima che i video arrivassero: **piu' 5,95 per cento**. La cartella
`brand_assets/maestri/` ne porta **7.251.097**, cioe' i tre filmati piu' i 1.679
byte del suo LEGGIMI. I tre pesi maggiori restano quelli di allora, il mazzo dei
tarocchi con 25,07 MiB, gli Angeli con 23,82 e i ritratti VIP con 12,14. **E il
peso degli asset non e' il peso dell'archivio**: l'APK della 2207 pesa
**173.822.285 byte**, letto dal file costruito.
