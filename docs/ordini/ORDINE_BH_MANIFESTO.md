...
- **BH.05** Antifrode. CHIUSA, con le regole enumerate: (1) LA LAPIDE DEL BENVENUTO: quando il premio viene pagato a una email, la sua impronta SHA-256 (indirizzo normalizzato, mai in chiaro) entra in lapidi_del_benvenuto, collezione FUORI dal ramo utente: la cancellazione non la tocca (recursiveDelete lavora solo su users/uid), le regole Firestore la chiudono a ogni client, e la guardia server pretende che cancellaIlCerchio non la nomini mai; un'email che ha consumato il benvenuto non lo riceve piu', con qualunque account; (2) IL RETROFIT: chi ha avuto il benvenuto quando era dote di nascita riceve la lapide alla prima sincronia registrata, cosi' anche i Cerchi vecchi (compreso quello del fondatore) sono coperti; (3) IL PREMIO SOLO A EMAIL VERIFICATA sul provider password (il premio non si compra con un indirizzo inventato); (4) le difese gia' in piedi restano (uid solo dal token, idempotenza per movimento, tetto condivisioni 3 al giorno, regole write-if-false). LA BASE GIURIDICA, dichiarata: legittimo interesse antifrode su dato pseudonimizzato, scritta nella privacy policy di BH.07; il sale opzionale dell'impronta arriva dall'ambiente (BENVENUTO_PEPPER, da mettere in Secret Manager al lancio: il segreto non sta nel codice). LIMITI DICHIARATI, non nascosti: le email usa e getta non sono bloccate (una lista di domini scartabili e' materia del lancio), nessuna impronta del dispositivo, nessun tetto di creazione account per IP (richiederebbe blocking functions): dichiarati per la revisione con la misura in mano. Prove server 42 verdi.
- **BH.06** Il menu utente riordinato e completo. CHIUSA: la lista madre ha l'ordine dichiarato (profilo, nascita, custodia, verifica email, cambia parola, abbonamento, notifiche, impostazioni, esci) e in fondo la porta nuova Privacy e dati, il sottomenu con la policy, lo scarico dei dati e le due cancellazioni; il vecchio anticipo Privacy e' diventato una porta vera e il parametro degli anticipi, rimasto senza voci, e' uscito. LE CANCELLAZIONI PROTETTE: tre passi (la spiegazione storica, il perche' con quattro ragioni e la scelta Preferisco non dirlo che non scrive niente, l'ultima conferma col bottone nel rosso di casa), il perche' viaggia al server nel CONGEDO (collezione fuori dal ramo utente, anonima per costruzione: la guardia pretende che non porti l'uid, si scrive prima di cancellare o non ci sarebbe piu' nessuno a scriverlo, non blocca mai il diritto); il sottotitolo dell'oblio non promette piu' i trenta giorni tolti da BE.07 e il dialogo dell'oblio avverte che il benvenuto non si ripetera'. E UN DIFETTO GRAVE PREESISTENTE, trovato dalla ricognizione e curato: deleteAllData passava da cancellaIlCerchio, quindi TUTTE le strade che promettevano "l'account resta tuo" (la voce azzera e quella delle Impostazioni) cancellavano anche l'ACCOUNT; ora passa da azzeraIDati e la guardia lo tiene fermo. Guardie: test/il_menu_ordinato_e_la_cancellazione_protetta_test.dart, la prova del menu senza voci morte rimirata sulla porta vera.
- **BH.07** La privacy policy. CHIUSA: nasce core/legal/privacy_policy.dart (il testo come dato, casa unica) con dieci sezioni scritte per il GDPR sulle cose che l'app fa DAVVERO: dati trattati, sensori che restano sul telefono, basi giuridiche, l'AI dichiarata (Gemini su Vertex, contenuti inviati a Google Cloud in regione europea, nessuna decisione automatizzata con effetti legali), dove vivono i dati (europe-west1), conservazione e cancellazione immediata, la prevenzione abusi con l'impronta SHA-256 che sopravvive all'oblio e il congedo anonimo, i diritti col reclamo al Garante, i minori di 16 anni, gli aggiornamenti datati; contatto info@esotericircle.com (un indirizzo privacy@ dedicato e' una scelta del fondatore per il lancio). La pagina PrivacyPolicyScreen monta il testo e il sottomenu la apre. LA GUARDIA ANCORA LE AFFERMAZIONI AL CODICE (test/la_policy_dice_il_vero_test.dart): impronta dichiarata e usata, nessuna cancellazione ad attesa, regione promessa e fissata: se il codice cambia, la prova cade prima che la pagina diventi una bugia pubblicata.
- **BH.08** Suite, build e consegna. CHIUSA: suite finale 3.442 verdi e DUE rossi dichiarati (l'attribuzione cieca, rosso di legge di AC.09, rimisurata in quest'ordine dal PC del fondatore: 76,7 per cento contro la soglia 85, Aura perfetta, Caligo scambiato per Aura nel 50 per cento dei giri e Medora nel 20: la cura della voce di Caligo e' proposta come ordine dedicato; e questa guardia stessa, rossa finche' questa voce era aperta). Prove server 42 verdi, deploy fatto due volte (statoDelCerchio, azzera e oblio; poi il congedo con la chiave ragione). Analyze a zero. Build 0.1.0+2203 (186.980.954 byte), PROVA DI ACCENSIONE VERA sul telefono del fondatore collegato via USB (processo vivo, primo fotogramma disegnato, nessun FATAL EXCEPTION, numero 2203 letto dal dispositivo): la prima accensione reale dopo undici salti dichiarati. Consegnata su App Distribution a cloud@esotericircle.app, release 6kr36i0e7rhn0, registro da 2202 a 2203.
